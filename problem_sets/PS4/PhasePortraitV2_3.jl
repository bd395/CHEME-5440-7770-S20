...
function phaseplot(model, x1lim, x2lim; clines=true, xinit=(), t=(0.0,50.0),
    ...
    if norm
        ...
        #Plot
        plt1 = quiver(x, y, quiver=(u_norm, v_norm), c=:grey, title = "n = 2", xaxis=("x1",
            (x1lim[1], x1lim[2])), yaxis=("u",(x2lim[1], x2lim[2])))
    else
    ...
    if clines
            ...
            contour!(x1_range, x2_range, v, lw=3, linestyle=:dot, c=:red,
                    levels=[0], colorbar = :none, title="n = 2", xaxis=("u", (x1lim[1], x1lim[2])),
    ...
    for i in xinit
        ...
        plot!(sol, vars=(1,2), lw=2, c=:black, legend = :none, title="n = 2", xaxis=("u",
            (x1lim[1], x1lim[2])), yaxis=("v",(x2lim[1], x2lim[2])))
    end
